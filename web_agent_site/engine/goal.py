...
def get_goals(all_products, product_prices, product_item_dict, new_tasks_path, human_goals=True):
    if human_goals:
        return get_human_goals_new(all_products, product_prices, product_item_dict, new_tasks_path)
    else:
        return get_synthetic_goals(all_products, product_prices, new_tasks_path)
    
def get_human_goals_new(all_products, product_prices, product_item_dict, new_tasks_path):
    goals = []
    cnt = 0

    with open(new_tasks_path) as f:
        new_tasks = json.load(f)
    
    for new_task in new_tasks:
        goal = []
        now_user_profile = new_task["user_profile"]
        for ins in new_task["instruction_group"]:
            asin       = ins["asin"]
            if asin not in product_item_dict: continue
            item       = product_item_dict[asin]
            ...
            goal.append({
                ...
                'instruction_text': now_user_profile + "|||" + ins['instruction'].strip('.') + price_text,
                'attributes': ins['instruction_attributes'],
                'price_upper': price_upper,
                'goal_options': ins['instruction_options'],
                'instruction_history': ins['instruction_history'],
            })
